...
def export_all_results(simulation_results, cvr_results, cvr_params, heston_params,
                       sim_params, output_dir='cvr_output'):
    """
    ...
    export_summary(cvr_results, cvr_params, heston_params, sim_params,
                   f"{output_dir}/summary_{timestamp}.csv")

    # 2. Export all payoffs
    export_payoffs(cvr_results, f"{output_dir}/payoffs_{timestamp}.csv")

    # 3. Export sample price paths
    export_price_paths(simulation_results, f"{output_dir}/price_paths_{timestamp}.csv")

    # 4. Export sample variance paths
    export_variance_paths(simulation_results, f"{output_dir}/variance_paths_{timestamp}.csv")

    # 5. Export rolling averages
    export_rolling_averages(simulation_results, f"{output_dir}/rolling_avgs_{timestamp}.csv")

    # 6. Export trigger times and types
    export_trigger_data(simulation_results, cvr_results, f"{output_dir}/triggers_{timestamp}.csv")

    # 7. Export percentile analysis
    export_percentiles(cvr_results, f"{output_dir}/percentiles_{timestamp}.csv")

    print(f"\nAll data exported to {output_dir}/")
    print(f"{'=' * 70}\n")


def export_summary(cvr_results, cvr_params, heston_params, sim_params, filename):
